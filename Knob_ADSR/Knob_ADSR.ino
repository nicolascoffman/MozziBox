...
void updateControl(){
  long new1, new2, new3, new4;  //Allocate memory
  ...
  // Detect changes in knob values / update LCD
  ...
  if (noteDelay.ready() ){
    // Map knob value to ms value for envelope
    attack = map(knob1.read(), 100, -100, 10, 1000);
    decay = map(knob2.read(), 100, -100, 10, 1000);
    sustain = map(knob3.read(), 100, -100, 10, 1000);
    release_ms = map(knob4.read(), 100, -100, 10, 1000);

    envelope.setTimes(attack,decay,sustain,release_ms);
    envelope.noteOn();

    byte midi_note = rand(24)+48;
